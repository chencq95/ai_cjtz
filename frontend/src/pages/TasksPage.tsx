...
export default function TasksPage({ user }: { user: User }) {
  ...
  const createSchedule = useMutation({ mutationFn:(body:any)=>post('/v1/schedules',body), onSuccess:()=>{message.success('计划已保存');setScheduleOpen(false);scheduleForm.resetFields();client.invalidateQueries({queryKey:['schedules']})},onError:(e:Error)=>message.error(e.message || '计划保存失败') })
  ...
  return <>
    ...
    <Modal title="新建调度计划" open={scheduleOpen} onCancel={()=>{setScheduleOpen(false);scheduleForm.resetFields()}} onOk={()=>scheduleForm.submit()} okText="保存计划" cancelText="取消" confirmLoading={createSchedule.isPending} destroyOnClose><Form form={scheduleForm} layout="vertical" onFinish={values=>createSchedule.mutate(values)} validateTrigger="onBlur"><Form.Item name="name" label="计划名称" rules={[{required:true,message:'请输入计划名称'},{max:160,message:'计划名称不能超过 160 个字符'}]}><Input placeholder="例如：每日增量采集" autoFocus/></Form.Item><Form.Item name="cron_expression" label="Cron 表达式" rules={[{required:true,message:'请输入 Cron 表达式'},{pattern:/^\\S+(?:\\s+\\S+){4}$/,message:'请输入 5 段 Cron 表达式，例如 30 2 * * *'}]}><Input className="mono" placeholder="30 2 * * *"/></Form.Item><Space align="start"><Form.Item name="timezone" label="时区" rules={[{required:true,message:'请输入时区'}]}><Input/></Form.Item><Form.Item name="mode" label="模式" rules={[{required:true}]}><Select style={{width:140}} options={[{value:'incremental',label:'增量'},{value:'full',label:'完整'}]}/></Form.Item><Form.Item name="enabled" label="启用" valuePropName="checked"><Switch/></Form.Item></Space><Form.Item name="platform_ids" label="平台范围"><Select mode="multiple" allowClear placeholder="不选择表示全部平台" options={(platforms.data||[]).map(p=>({value:p.id,label:`${p.id}. ${p.name}`}))}/></Form.Item><Form.Item name="max_pages" label="每个平台最多抓取页数"><InputNumber min={1} style={{width:'100%'}} placeholder="系统默认"/></Form.Item></Form></Modal>
